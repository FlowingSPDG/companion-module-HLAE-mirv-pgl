...
class mirvpgl {
    constructor(port, path_in) {
        ...
        this.wss.on('connection', function (newWs) {
            if (self.ws) {
                self.ws.close();
                self.ws = newWs;
            }
                                        ...
                                        for (var eventName in enrichments) {
                                            for (var keyName in enrichments[eventName]) {
                                                ...
                                                for (var i = 0; i < arrEnrich.length; ++i) {
                                                    self.ws.send(new Uint8Array(Buffer.from(`exec\0mirv_pgl events enrich eventProperty "${arrEnrich[i]}" "${eventName}" "${keyName}"\0`, 'utf8')), { binary: true });
                                                }
                                            }
                                        }
                                        self.ws.send(new Uint8Array(Buffer.from('exec\0mirv_pgl events enabled 1\0' // enable event
                                        , 'utf8')), { binary: true });
                                        self.ws.send(new Uint8Array(Buffer.from('transEnd\0', 'utf8')), { binary: true });
                                    }
                                    break;
                                case 'dataStart':
                                    break;
...
//# sourceMappingURL=mirvpgl.js.map